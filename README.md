...
R48已有第一关实玩、招聘/建房/音效/槽位保存观察；所选窗口约3次软件画面提交/秒，
绘制和模拟计时仍是优化重点，恢复存档与大关容量尚未证明。
R49产品候选 `7f1abe1` 修正方向/白框、完善L视野切换，按CorsixTH原有五档调速，补充实际计时观测。
最新留存回执记录R49上传及读回通过，R49实玩尚待确认；R50整理期间没有连接设备。
R50 `9fc7d5f` 已完成首批结构整理：上游接入工具分层、固定测试资料抽离、真实双屏视野组件、
统一3DS编译文件登记。完整主机回归、ARM代码检查及真实交叉构建通过，R50尚未上传。
这轮结果证明拆分后的电脑侧链路可用；帧率改善、大关容量和实体输入继续按滚动表验收。
...
- 普通地图：十字键移医院镜头，摇杆移观察白框，R+十字键精确移点击光标；R49修复待真机复验。
