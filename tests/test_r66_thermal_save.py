...
class R66ThermalSaveTests(unittest.TestCase):
    def test_existing_r65_report_migrates_once(self):
        ...
        with tempfile.TemporaryDirectory(prefix='cth-r66-migrate-') as name:
            ...
            self.assertIn('TH3DS.diagnostic_line(report)', first)
...
''')
    ...
    def test_native_diagnostic_failure_matrix_retry_and_commit_boundary(self):
        # Reuse the full existing real-file 23-failure matrix with the native
        # outlet available. The print fallback remains covered by its own test.
        ...
        def instrument(instance, script):
            ...
            script = script.replace(anchor, '''native.diagnostic_line=function(message)
...
''')
            ...
            self.assertEqual(script.count(anchor), 1)
            script = script.replace(anchor, '''-- A closed-file report cannot certify the later commit.
...
assert(#messages==reports_before+1,'closed temporary file report precedes failed commit')
''' + anchor)
            script = script.replace('stream_reports==23', 'stream_reports==24')
