...
统一计划与验收状态见 [滚动计划表](docs/ROLLING_PLAN.md)。R47 已有第一关实玩、
招聘/建房/音效/保存退出的观察；约 2.4 次/秒的画面更新仍远低于流畅目标。
当前 R48 优先减少真实绘制开销、补全输入采样、修正监控；同时提供视野切换与存档槽。
R48 真机性能、恢复存档和长期稳定性须分别验证。完整原版数据由用户合法提供，
不包含在公开仓库。工具链与资源转换缓存应复用。

- 上屏：白框中的清晰局部（400×240）；R48 增加 480×288 查看范围的广角档。
- 下屏：当前 640×480 画布的半尺寸总览，触控笔直接点击/拖动，保留原游戏界面。
- R48 普通地图：十字键移医院镜头，摇杆移观察白框，R+十字键精确移点击光标。
- 字体、音乐和后期关卡容量按滚动表推进；GPU 利用率尚无有效测量。

## 历史方案与版本记录

以下为早期阶段记录；功能描述和通过结论须结合对应版本，当前路线按上表核对。
0.4 阶段目标形态为：
