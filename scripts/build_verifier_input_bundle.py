...
def verify_completed_bundle(root: Path) -> dict[str, Any]:
    ...
    heads = run("/usr/bin/git", "bundle", "list-heads",
                str(root / "candidate/candidate.bundle")).splitlines()
    if heads != [str(identity.get("head")) + " HEAD"]:
        raise BundleError("CANDIDATE_BUNDLE_REFSET_INVALID")
