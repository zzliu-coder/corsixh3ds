...
# Existing producer input roles and frozen file-byte digests, not derived result
# digests or directory-tree digests (evidence_protocol_adversarial.py:32-35).
FROZEN_ROLE_DIGESTS = {
...
BUNDLE_ROLES = set(FROZEN_ROLE_DIGESTS) | {
    "candidate_transport", "source_archive", "cross_dependencies",
    "python_runtime_3_9_25", "python_runtime_3_14_6",
    "python_wheelhouse_3_9_25", "python_wheelhouse_3_14_6",
    "old3ds_toolchain", "public_ci_environment",
}


def role_checks(value, expected_roles, label):
    rows = seq(value, label)
    byrole, paths = {}, set()
    for row in rows:
        obj(row, label + ".role")
        name = string(row.get("role"), label + ".role name")
        path = safe_relative(row.get("bundle_relative_path")).as_posix()
        value = hash_value(row.get("sha256_or_tree_digest"), label + ".digest")
        require(name not in byrole and path not in paths,
                "FRESH_BUNDLE_BINDING_MISMATCH", label + ": duplicate role/path")
        if name in FROZEN_ROLE_DIGESTS:
            require(value == FROZEN_ROLE_DIGESTS[name], "FRESH_BUNDLE_BINDING_MISMATCH", label + ": frozen " + name)
        byrole[name] = row
        paths.add(path)
    require(set(byrole) == expected_roles, "FRESH_BUNDLE_BINDING_MISMATCH", label + ": exact roles")
    return byrole


def recorded_root(value, label):
    # These are recorded producer paths; never resolve them on the consumer host.
    path = pathlib.PurePosixPath(string(value, label))
    require(path.is_absolute() and path.as_posix() == value and ".." not in path.parts
            and "\x00" not in value and "\\" not in value,
            "FRESH_MANIFEST_BINDING_MISMATCH", label + ": normalized absolute path")
    return path
...
def semantic_checks(root: pathlib.Path, manifest: dict, expected: dict) -> None:
    ...
    declared_roles = seq(bundle.get("roles"), "bundle.roles")
    for role in declared_roles:
        string(role, "bundle role")
    require(len(declared_roles) == len(set(declared_roles)) and set(declared_roles) == BUNDLE_ROLES,
            "FRESH_BUNDLE_BINDING_MISMATCH", "envelope exact roles")
    roles = role_checks(final_rehash.get("checked"), set(declared_roles), "final rehash")
    preflight = obj(r4_rows[0].get("evidence"), "R4C01.evidence")
    preflight_rehash = obj(preflight.get("preflight_rehash"), "preflight rehash")
    preflight_roles = role_checks(preflight_rehash.get("checked"), set(declared_roles), "preflight rehash")
    require(preflight_roles == roles, "FRESH_BUNDLE_BINDING_MISMATCH", "preflight/final role mapping")
    inputs = obj(preflight.get("inputs"), "preflight.inputs")
    require(set(inputs) == set(roles), "FRESH_BUNDLE_BINDING_MISMATCH", "preflight input roles")
    for name, role in roles.items():
        source = obj(inputs[name], "input " + name)
        require(source.get("bundle_relative_path") == role["bundle_relative_path"] and source.get("readonly") is True,
                "FRESH_BUNDLE_BINDING_MISMATCH", "input path " + name)
    session_root = recorded_root(preflight.get("session_root_realpath"), "session root")
    bundle_root = recorded_root(preflight.get("bundle_root_realpath"), "bundle root")
    require(preflight_rehash.get("bundle_root") == final_rehash.get("bundle_root") == str(bundle_root),
            "FRESH_BUNDLE_BINDING_MISMATCH", "rehash roots")
    ...
    # Follow the frozen producer's configure -> build -> execution sources.
    # Directory/profile labels alone do not establish sanitizer configuration.
    source = str(session_root / "00-preflight/candidate-detached/tests/runtime_core_v2")
    cmake_identity = None
    profile_builds = {}
    canonical_h2 = None

    def source_row(stage, argv, deps, output):
        row = obj(byid.get(stage), stage)
        require(seq(row.get("argv"), stage + ".argv") == argv
                and row.get("executable_relative_path") == argv[0]
                and row.get("argument_roles") == [a[2:] for a in argv if a.startswith("--")]
                and row.get("dependency_ids") == deps and row.get("exit_code") == 0
                and row.get("owner") == "validation-task"
                and row.get("input_role") is None and row.get("output_root") == output,
                "FRESH_H2_RECORD_MISMATCH", stage + ".source")
        for dep in deps:
            require(utc(byid[dep]["ended_at"], dep) <= utc(row["started_at"], stage),
                    "FRESH_H2_RECORD_MISMATCH", stage + ".dependency time")
        return row

    for profile in ("sanitized", "non_sanitized"):
        sanitized = profile == "sanitized"
        prefix = "r4.n20_produce.invocation." if sanitized else "r4.n90_final_audit.h2_plain_"
        configure_id = prefix + ("configure-red" if sanitized else "configure")
        build_id = prefix + ("build-red" if sanitized else "build")
        build = str(session_root / ("20-canonical-run/build_red" if sanitized else "90-final-audit/h2-plain-build"))
        configure = obj(byid.get(configure_id), configure_id)
        cmake = str(recorded_root(configure.get("executable_relative_path"), "configure tool"))
        tool_identity = (cmake, hash_value(configure.get("executable_sha256"), "configure tool digest"))
        require(cmake_identity in (None, tool_identity), "FRESH_H2_RECORD_MISMATCH", "profile configure tool")
        cmake_identity = tool_identity
        dependencies = ["r4.n10_policy" if sanitized else "r4.n81_cycle_acceptance"]
        output = None if sanitized else build
        configure_argv = [cmake, "-S", source, "-B", build,
                          "-DCTH3DS_ENABLE_SANITIZERS=" + ("ON" if sanitized else "OFF"),
                          "-DCTH3DS_WARNINGS_AS_ERRORS=ON",
                          "-DCMAKE_BUILD_TYPE=" + ("Debug" if sanitized else "RelWithDebInfo")]
        if sanitized:
            configure_argv.append("-DCMAKE_EXPORT_COMPILE_COMMANDS=ON")
        source_row(configure_id, configure_argv, dependencies, output)
        targets = (["cth3ds-red-h1-level-requires-package"] if sanitized else []) + ["cth3ds-red-h2-transition-lease-escape"]
        built = source_row(build_id, [cmake, "--build", build, "--parallel", "--target"] + targets,
                           dependencies if sanitized else [configure_id], output)
        require(hash_value(built.get("executable_sha256"), "build tool digest") == tool_identity[1]
                and utc(configure["ended_at"], configure_id) <= utc(built["started_at"], build_id),
                "FRESH_H2_RECORD_MISMATCH", profile + ".configure/build")
        profile_builds[profile] = built
        if sanitized:
            anchor_id = prefix + "RH07-H2"
            anchor = obj(byid.get(anchor_id), anchor_id)
            anchor_argv = seq(anchor.get("argv"), "canonical H2 argv")
            require(len(anchor_argv) == 5, "FRESH_H2_RECORD_MISMATCH", "canonical H2 argv length")
            source_row(anchor_id, [build + "/cth3ds-red-h2-transition-lease-escape", "--run-id",
                                  string(anchor_argv[2], "canonical H2 run id"), "--fault", "after-first-staged-acquire"],
                       dependencies, None)
            require(utc(built["ended_at"], build_id) <= utc(anchor["started_at"], anchor_id),
                    "FRESH_H2_RECORD_MISMATCH", "canonical build/execution time")
            for row in (configure, built, anchor):
                require(utc(byid["r4.n20_produce"]["started_at"], "produce start") <= utc(row["started_at"], "source start")
                        and utc(row["ended_at"], "source end") <= utc(byid["r4.n20_produce"]["ended_at"], "produce end"),
                        "FRESH_H2_RECORD_MISMATCH", "canonical source interval")
            canonical_h2 = (anchor["executable_relative_path"], hash_value(anchor.get("executable_sha256"), "canonical H2 digest"))
    ...
    for profile in ("sanitized", "non_sanitized"):
        ...
        for index in range(1, 21):
            ...
            totals = {}
            for key, length, slot in (("pool_bytes",7,2),("backend_bytes",2,0)):
                ...
                for array in arrays:
                    for value in array:
                        integer(value, key)
                totals[key] = arrays[1][slot] - arrays[0][slot]
            ...
            executable = str(session_root / build / "cth3ds-red-h2-transition-lease-escape")
            argv = seq(j.get("argv"), "H2 argv")
            require(argv == [executable, "--run-id", run_id, "--fault", "after-first-staged-acquire"]
                    and j.get("executable_relative_path") == executable
                    and j.get("argument_roles") == ["run-id", "fault"]
                    and j.get("dependency_ids") == ["r4.n81_cycle_acceptance"]
                    and j.get("owner") == "validation-task"
                    and j.get("input_role") is None and j.get("output_root") is None
                    and observation.get("schema") == "cth3ds.runtime-core-raw-observation/v4"
                    and observation.get("gate_id") == "RH07-H2"
                    and observation.get("oracle_id") == "H2_TRANSITION_CAPABILITY_ROLLBACK"
                    and observation.get("stage_id") == "C3",
                    "FRESH_H2_RECORD_MISMATCH", name + ".process source")
            executable_identity = (executable, hash_value(j.get("executable_sha256"), "H2 executable digest"))
            require(utc(profile_builds[profile]["ended_at"], "profile build") <= utc(j["started_at"], "H2 execution")
                    and (profile != "sanitized" or (executable_identity == canonical_h2
                         and utc(anchor["ended_at"], "canonical H2") <= utc(j["started_at"], "H2 execution"))),
                    "FRESH_H2_RECORD_MISMATCH", name + ".build/canonical source")
            require(profile_executables.setdefault(profile, executable_identity) == executable_identity,
                    "FRESH_H2_RECORD_MISMATCH", name + ".profile executable")
            passed += 1
    ...
    for index, row in enumerate(r4_rows, 1):
        ...
        if index == 1:
            ...
            candidate_role = roles["candidate_transport"]
            require(transport.get("kind") == "head-bundle"
                    and transport.get("bundle_sha256") == transport.get("source_sha256") == candidate_role["sha256_or_tree_digest"]
                    and transport.get("source_realpath") == str(bundle_root / candidate_role["bundle_relative_path"])
                    and transport.get("normalized_repo_realpath") == str(session_root / "00-preflight/candidate-detached")
                    and transport.get("advertised_refs") == [{"name": "HEAD", "oid": expected["head"]}],
                    "FRESH_BUNDLE_BINDING_MISMATCH", "candidate transport source")
            dag_source = obj(evidence.get("execution_dag"), "preflight.execution_dag")
            require(dag_source.get("bundle_relative_path") == roles["execution_dag"]["bundle_relative_path"]
                    and dag_source.get("sha256") == roles["execution_dag"]["sha256_or_tree_digest"],
                    "FRESH_BUNDLE_BINDING_MISMATCH", "DAG source")
