...
这是一个面向 **Old Nintendo 3DS** 的 CorsixTH 0.70.1 移植工程。

## 当前入口（2026-09-09，R63 施工候选）

当前计划：[R63 状态正确性与五框架滚动施工表](docs/R63_PERFORMANCE_PLAN.md)。
故障与交接：[R63 外部审查上下文](docs/R63_EXTERNAL_REVIEW.md)。

R62 两轮设备日志确认员工采样的严格变量错误，恢复路径会把员工 `ticks=false` 保存。
R63 修复 Staff/BottomPanel 平台绑定，给明确命名的 R62 存档副本提供受限恢复，
并为 benchmark 加入人员更新、timer、错误和保存重载状态检查。原始 Slot1 保留。
同批优化包括无垃圾时跳过周边查询、整数编码无临时堆分配、同容量双候选文字缓存。
电脑回归、ARM 构建和真机结果分别登记，R63 设备结果尚待验证。

GPU 绘制已在后续版本接通且有真实设备改善；健康 R61 特定场景约 11.7～11.9 FPS。
R62 错误后人员停更的帧率不可用于比较；30 FPS+ 和繁忙医院容量仍为未达成目标。
中文、可选播报语音和流式 PCM 音乐已纳入常态资源路径；继续保留现有双屏与操作方案。

## R53 历史入口（2026-09-07）

历史实测与外部审查：[R53 两次运行、性能和 emergency 滚图诊断](docs/audits/r53-device-20260907/README.md)。
受测产品 `367a887`；两次 GPU 启动自检均 1024/1024 像素失败，自动回退软件绘制。
Normal 有效样本约 2.163 次呈现/秒，30FPS 目标尚未达到。
接受 emergency 后计时小窗被识别为 Dialog、阻止边缘滚图的路径已在电脑端复现。
两轮保存提交和正常退出有记录；完整存读档、忙医院容量和 GPU 画面仍需继续验证。
本次发布更新源代码历史、完整日志和复现方法，不附原版游戏资料或私人存档。

统一计划与验收状态见 [滚动计划表](docs/ROLLING_PLAN.md)，代码职责与下一步拆分见
[当前结构与整理计划](docs/STRUCTURE_PLAN.md)，实际迭代/测试步骤见
[增量开发与真机候选](docs/DEVELOPMENT_LOOP.md)。
R47–R51 已有招聘、建房、音效和槽位保存的实玩记录；R51 载入旧 Slot3 成功，
覆盖保存遇到奇数宽弹窗坐标错误，原存档未改。R51 普通速度稳定样本约 2.24 次呈现/秒，
绘制与双屏约占完整间隔 81.6%，实际游戏时间也明显落后。30FPS+ 仍为待达成目标。
R52 候选接入 GPU 画布与双屏输出、地图温度缓存、分离调度和自动存档副本基准，
并修复覆盖弹窗坐标；具体边界和集中测试见 [R52 GPU 候选](docs/R52_GPU_CANDIDATE.md)。
当前实现、完整主机回归、交叉编译、部署与真机观察分别记账，不能互相代替。
R52 `cea5fc1` 已于2026-09-07完成主机/交叉检查及增量安装读回；
216项Python通过、1项明确Mac文件系统跳过，Debug/Release各152项C++通过。
后续 R52 真机花屏；R53 主机纹理对照通过，实体启动自检失败并回退。
本次 30FPS 目标为 `FAIL`；GPU 正确画面与完整覆盖保存恢复为 `NOT_PROVEN`。
完整原版数据由用户合法提供，不包含在公开仓库。工具链与资源转换缓存继续复用。

- 上屏：白框中的清晰局部（400×240）；R48 增加 480×288 查看范围的广角档。
- 下屏：当前 640×480 画布的半尺寸总览，触控笔直接点击/拖动，保留原游戏界面。
- 普通地图：摇杆移动白框，到边缘后滚地图；触控笔独占鼠标。十字键不移动视野或鼠标。
- 字体、音乐和后期关卡容量按滚动表推进；GPU 利用率尚无有效测量。

## 历史方案与版本记录

以下及旧架构/外部审核文件为早期阶段记录；功能描述和通过结论须结合对应版本，
当前结构、路线与验收以本页入口链接为准。
0.4 阶段目标形态为：
