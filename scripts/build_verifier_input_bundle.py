...
def product_fingerprint(repo: Path) -> dict[str, Any]:
    paths = [line for line in git(repo, "ls-files").splitlines() if line not in ALLOWLIST]
    rows = []
...
def create_bundle(args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        ...
        heads = run("/usr/bin/git", "bundle", "list-heads", str(candidate_bundle)).splitlines()
        if heads != [head + " HEAD"]:
        ...
        add_input(inputs, staged, "candidate_transport", "validation-task", repo,
                  ...
                  {**candidate_identity, "product_fingerprint": product_fingerprint(repo)})
