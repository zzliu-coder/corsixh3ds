...
class BuildScriptTests(unittest.TestCase):
    def test_verifier_git_calls_scope_safe_directory_without_home(self) -> None:
        spec = importlib.util.spec_from_file_location(
            "safe_directory_test_driver", ROOT / "scripts/verifier_driver.py"
        )
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with mock.patch.object(driver.subprocess, "run", return_value=completed) as run:
                self.assertEqual(driver.run_git(repo, "status", "--porcelain"), "clean")
            ...
            self.assertEqual(environment, {"PATH": "/usr/bin:/bin", "LC_ALL": "C", "TZ": "UTC"})
