...
class VersionAlignmentTests(unittest.TestCase):
    def test_public_and_embedded_versions_match(self) -> None:
        ...
        self.assertIn('"R56 " + state.build_tag', runtime)
        self.assertIn('"CORSIXTH R56 "', runtime)
        self.assertIn('revision=R56', runtime)
        self.assertTrue(r'memcmp(magic,"R56\n",4)' in runtime,
                        'one-shot marker must match R56 exactly')
        self.assertTrue('sdmc:/3ds/corsixth/benchmark-used-r56.txt' in runtime,
                        'consumed benchmark marker must identify R56')
