...
def make_fresh_sources(root: Path):
  ...
  },
  {
    ...
    # Actual configure/build roles retained by the frozen producer, with
    # relocatable recorded roots and a prior canonical H2 execution source.
    for profile in ("sanitized", "non_sanitized"):
        ...
        build = str(session.absolute() / ("20-canonical-run/build_red" if sanitized else "90-final-audit/h2-plain-build"))
        deps = ["r4.n10_policy" if sanitized else "r4.n81_cycle_acceptance"]
        configure = ["/usr/bin/cmake", "-S", str(session.absolute() / "00-preflight/candidate-detached/tests/runtime_core_v2"),
                     "-B", build, "-DCTH3DS_ENABLE_SANITIZERS=" + ("ON" if sanitized else "OFF"),
                     "-DCTH3DS_WARNINGS_AS_ERRORS=ON", "-DCMAKE_BUILD_TYPE=" + ("Debug" if sanitized else "RelWithDebInfo")]
        if sanitized:
        ...
        commands = [(configure_id, configure, deps),
                    (build_id, ["/usr/bin/cmake", "--build", build, "--parallel", "--target"] + targets,
                     deps if sanitized else [configure_id])]
        if sanitized:
            commands.append((prefix + "RH07-H2", [build + "/cth3ds-red-h2-transition-lease-escape", "--run-id",
                             "canonical-run", "--fault", "after-first-staged-acquire"], deps))
        for stage, argv, dependencies_for_stage in commands:
            row = invocation_row(stage, dependencies_for_stage)
            row.update(argv=argv, executable_relative_path=argv[0],
                       executable_sha256=("c" if stage.endswith("RH07-H2") else "e") * 64,
                       argument_roles=[a[2:] for a in argv if a.startswith("--")],
                       owner="validation-task", input_role=None, output_root=None if sanitized else build)
            journal.append(row)
    for profile in ("sanitized", "non_sanitized"):
            ...
            def relocate(documents, journal):
                ...
                def moved(value):
                    if isinstance(value, str):
                        return value.replace(prior_session, e["session_root_realpath"]).replace(prior_bundle, e["bundle_root_realpath"])
                    if isinstance(value, list):
                        return [moved(item) for item in value]
                    if isinstance(value, dict):
                        return {key: moved(item) for key, item in value.items()}
                    return value
                journal[:] = moved(journal)
            source_case("relocated-complete-source", relocate, accept=True)

            def profile_change(profile, change):
                def apply(documents, journal):
                    byid = {j["stage_id"]: j for j in journal}
                    prefix = "r4.n20_produce.invocation." if profile == "sanitized" else "r4.n90_final_audit.h2_plain_"
                    configure = byid[prefix + ("configure-red" if profile == "sanitized" else "configure")]
                    built = byid[prefix + ("build-red" if profile == "sanitized" else "build")]
                    anchor = byid["r4.n20_produce.invocation.RH07-H2"]
                    observed = [byid[f"r4.n90_final_audit.h2_{profile}_{i:02d}"] for i in range(1, 21)]
                    change(configure, built, anchor, observed, journal)
                return apply

            for profile in ("sanitized", "non_sanitized"):
                changes = [
                    ("flag", lambda c, b, a, rows, js: c["argv"].__setitem__(5, "-DCTH3DS_ENABLE_SANITIZERS=" + ("OFF" if c["argv"][5].endswith("=ON") else "ON"))),
                    ("extra-override", lambda c, b, a, rows, js: c["argv"].append("-DCTH3DS_ENABLE_SANITIZERS=ON")),
                    ("source", lambda c, b, a, rows, js: c["argv"].__setitem__(2, "/wrong/tests/runtime_core_v2")),
                    ("output-pair", lambda c, b, a, rows, js: (c["argv"].__setitem__(4, "/wrong/build"), b["argv"].__setitem__(2, "/wrong/build"))),
                    ("tool-pair", lambda c, b, a, rows, js: (c["argv"].__setitem__(0, "/other/cmake"), c.update(executable_relative_path="/other/cmake"))),
                    ("tool-digest-pair", lambda c, b, a, rows, js: (c.update(executable_sha256="0" * 64), b.update(executable_sha256="0" * 64))),
                    ("target", lambda c, b, a, rows, js: b["argv"].__setitem__(-1, "other-target")),
                    ("owner", lambda c, b, a, rows, js: c.update(owner="other-task")),
                    ("input-role", lambda c, b, a, rows, js: b.update(input_role="other")),
                    ("output-root", lambda c, b, a, rows, js: b.update(output_root="/wrong/build")),
                    ("dependency", lambda c, b, a, rows, js: b.update(dependency_ids=[])),
                    ("argument-role", lambda c, b, a, rows, js: b.update(argument_roles=[])),
                    ("configure-time", lambda c, b, a, rows, js: c.update(ended_at="2026-09-05T00:00:01Z")),
                    ("build-time", lambda c, b, a, rows, js: b.update(ended_at="2026-09-05T00:00:01Z")),
                    ("missing-configure", lambda c, b, a, rows, js: js.remove(c)),
                    ("missing-build", lambda c, b, a, rows, js: js.remove(b)),
                ]
                for label, change in changes:
                    source_case("r31-" + profile + "-" + label, profile_change(profile, change))
                for value_index, bad_value in enumerate((None, [], {}, True, 1, "wrong")):
                    source_case(f"r31-{profile}-argv-shape-{value_index}", profile_change(profile,
                        lambda c, b, a, rows, js, value=bad_value: c.update(argv=value)))

            canonical_changes = [
                ("all20-digest", lambda c, b, a, rows, js: [j.update(executable_sha256="0" * 64) for j in rows]),
                ("reverse-digest", lambda c, b, a, rows, js: a.update(executable_sha256="0" * 64)),
                ("reverse-path-pair", lambda c, b, a, rows, js: (a["argv"].__setitem__(0, "/other/h2"), a.update(executable_relative_path="/other/h2"))),
                ("time", lambda c, b, a, rows, js: a.update(ended_at="2026-09-05T00:00:01Z")),
                ("missing", lambda c, b, a, rows, js: js.remove(a)),
                ("fault", lambda c, b, a, rows, js: a["argv"].__setitem__(4, "other-fault")),
                ("owner", lambda c, b, a, rows, js: a.update(owner="other-task")),
            ]
            for label, change in canonical_changes:
                source_case("r31-canonical-" + label, profile_change("sanitized", change))
            source_case("r31-consistent-dynamic-digest", profile_change("sanitized",
                lambda c, b, a, rows, js: [j.update(executable_sha256="0" * 64) for j in [a] + rows]), accept=True)
